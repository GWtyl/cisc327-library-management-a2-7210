...
def test_add_book_valid_input(monkeypatch):
    ...
    monkeypatch.setattr("services.library_service.insert_book", lambda *args, **kwargs: True)
    success, message = ls.add_book_to_catalog("Test Book", "Test Author", "1234567890123", 5)
...
def test_add_book_invalid_isbn(monkeypatch):
    ...
    success, message = ls.add_book_to_catalog("Test Book", "Test Author", "123", 5)
...
def test_add_book_duplicate_isbn(monkeypatch):
    ...
    monkeypatch.setattr("services.library_service.get_book_by_isbn", lambda isbn: {"title": "Existing"})
    success, message = ls.add_book_to_catalog("Another Book", "Another Author", "1234567890123", 3)
    assert success is False
...
def test_borrow_book_success(monkeypatch):
    ...
    monkeypatch.setattr('services.library_service.get_patron_borrow_count', mock_get_count)
    monkeypatch.setattr('services.library_service.insert_borrow_record', mock_insert_borrow)
